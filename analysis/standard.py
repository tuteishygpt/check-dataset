...
def _get_result_audio(ds, result, path_index=None):
    ...
    return resolve_audio(ds, result, path_index=path_index)


def _has_audio_data(audio_data) -> bool:
    """Return whether a cached result contains usable audio data."""
    return has_valid_audio(audio_data)


def _validate_analysis_params(analysis_scope, recheck_problematic, limit_files, thinking_budget, similarity_threshold, temperature):
    """Coerce Gradio inputs to the correct types."""
    analysis_scope = normalize_analysis_scope(analysis_scope, recheck_problematic=recheck_problematic)
    ...
    return analysis_scope, limit_files, thinking_budget, similarity_threshold, temperature
...
def run_analysis(
    ...
    analysis_scope, limit_files, thinking_budget, similarity_threshold, temperature = (
        _validate_analysis_params(analysis_scope, recheck_problematic, limit_files, thinking_budget, similarity_threshold, temperature)
    )
...
def _run_recheck_analysis(
    ...
    ds = load_dataset_cached(dataset_name, None, hf_token, progress)
    path_index = utils.build_dataset_path_index(ds)
    ...
    for j, idx in enumerate(target_indices):
        ...
        merge_model_result(global_results[idx], model_name, hyp_text, score, norm_ref, norm_hyp, similarity_threshold)
...
def _run_fresh_analysis(
    ...
    if analysis_scope == ANALYSIS_SCOPE_PENDING and pending_indices:
        ...
        for j, idx in enumerate(pending_indices):
            ...
            merge_model_result(results[idx], model_name, hyp_text, score, norm_ref, norm_hyp, similarity_threshold)
            results[idx].update({"audio_array": audio_data, "sampling_rate": sampling_rate})
    ...
    limit = int(limit_files) if limit_files > 0 else None
    ds = load_dataset_cached(dataset_name, limit, hf_token, progress, decode_audio=False)
    ...
    for idx, item in enumerate(ds):
        ...
        if is_transcription_error(hyp_text):
            ...
            results.append(init_result_entry(idx, audio_path, ref_text, model_name))
            continue
        score, norm_ref, norm_hyp = utils.calculate_similarity(ref_text, hyp_text)

        entry = init_result_entry(idx, audio_path, ref_text, model_name)
        merge_model_result(entry, model_name, hyp_text, score, norm_ref, norm_hyp, similarity_threshold)
...
def _submit_hf_batch(hf_client, ds, batch_items):
    """Prepare audio and submit a batch to the HF ASR client.

    Items may carry pre-decoded audio (audio_data/sampling_rate keys) or
    a source_idx for on-the-fly decoding from ds.

    Returns the transcriptions dict, or empty dict on non-quota errors.
    Raises RuntimeError("QUOTA_EXCEEDED") on GPU quota exhaustion.
    """
    batch_audio = []
    for item in batch_items:
        if "audio_data" in item:
            batch_audio.append((item["idx"], item["audio_data"], item["sampling_rate"]))
        else:
    ...
    try:
        return hf_client.transcribe_batch(batch_audio)
...
def _collect_hf_results(batch_items, transcriptions, results, model_name, similarity_threshold, *, recheck=False):
    """Apply HF ASR transcriptions to results, return count of successful items."""
    transcribed_count = 0
    for item in batch_items:
        idx = item["idx"]
        ...
        if recheck:
            print(f"🔄 HF Updated: {results[idx].get('path')} | Score: {results[idx].get('score')} -> {score}")
            merge_model_result(results[idx], model_name, hyp_text, score, norm_ref, norm_hyp, similarity_threshold)
        else:
            results[idx].update({
...
def _run_hf_batch_loop(hf_client, ds, items_to_process, results, model_name, dataset_name,
                        similarity_threshold, progress, *, recheck=False, progress_base=0.1):
    """Process items in HF_BATCH_SIZE batches with rate-limit delays."""
    from core.state import get_stop_requested
    ...
    for batch_num in range(num_batches):
        ...
        progress_val = progress_base + (batch_num / num_batches) * (1 - progress_base)
        progress(progress_val, desc=f"Пакет {batch_num + 1}/{num_batches}: {len(batch_items)} файлаў (HF ASR)...")

        try:
            transcriptions = _submit_hf_batch(hf_client, ds, batch_items)
        except RuntimeError:
            print("🛑 Спыненне: перавышанне квоты Pro GPU. Захаванне...")
            break

        count = _collect_hf_results(batch_items, transcriptions, results, model_name, similarity_threshold, recheck=recheck)
        print(f"✅ Пакет {batch_num + 1}/{num_batches}: {count}/{len(batch_items)} транскрыбавана")
        set_global_results(results)
        save_results_csv(dataset_name, auto_prefix=False)
...
def _run_hf_fresh_analysis(
    ...
    if analysis_scope == ANALYSIS_SCOPE_PENDING and pending_indices:
        ...
        ds = load_dataset_cached(dataset_name, None, hf_token, progress, decode_audio=False)
        path_index = utils.build_dataset_path_index(ds)
        ...
        progress(0.1, desc=f"Апрацоўка {len(items_to_process)} файлаў (HF ASR)...")
        _run_hf_batch_loop(hf_client, ds, items_to_process, results, model_name,
                           dataset_name, similarity_threshold, progress)
        set_global_results(results)
        return generate_dashboard_outputs(similarity_threshold)
    ...
    for idx, item in enumerate(ds):
        ref_text = utils.get_record_text(item)
        all_items.append({
            "idx": idx, "source_idx": idx,
            "path": utils.get_audio_path(item), "ref_text": ref_text
        })

    results = [
        init_result_entry(item["idx"], item["path"], item["ref_text"], model_name, source_idx=item["source_idx"])
        for item in all_items
    ]
    set_global_results(results)

    _run_hf_batch_loop(hf_client, ds, all_items, results, model_name,
                       dataset_name, similarity_threshold, progress)
    set_global_results(results)
    return generate_dashboard_outputs(similarity_threshold)
...
def _run_hf_recheck_analysis(
    ...
    target_indices = get_analysis_target_indices(
        global_results, analysis_scope, similarity_threshold, limit_files=limit_files,
    )
    ...
    progress(0.1, desc=f"Пераправерка {len(items_to_process)} файлаў (HF ASR)...")
    _run_hf_batch_loop(hf_client, ds, items_to_process, global_results, model_name,
                       dataset_name, similarity_threshold, progress, recheck=True)
    return generate_dashboard_outputs(similarity_threshold)
